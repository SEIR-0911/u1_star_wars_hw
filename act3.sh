...
#after getting into the 'empire' directory:
...
mv chewbacca.txt millenium_falcon && mv han_solo.txt millenium_falcon && mv princess_leia.txt millenium_falcon && mv luke_skywalker.txt millenium_falcon
#is there any easier way to do this while still making sure not to grab vader?
...
#after navigating back to the empire directory
#ok so I messed this up the first time and ended up changing his name to empire. So here's what I actually did and how I fixed it:
#mv death_star/darth_vader.txt empire
#mv empire darth_vder.txt
